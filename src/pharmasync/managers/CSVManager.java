...
public class CSVManager {
    ...
    static {
        createFileIfNotExists(INVENTORY_FILE, "type,id,name,price,quantity,category,extra_field\n" +
                "rx,MED001,Amoxicillin,15.50,100,TABLET,Dr. Smith\n" +
                "otc,MED002,Ibuprofen,8.99,50,TABLET,false\n" +
                "otc,MED003,Cough Syrup,12.00,10,SYRUP,false\n");
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(INVENTORY_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (type.equalsIgnoreCase("rx")) {
                    inventory.add(new PrescriptionDrug(id, name, price, quantity, refCat, extra));
                } else {
                    boolean ageRestricted = Boolean.parseBoolean(extra);
                    inventory.add(new OverTheCounterDrug(id, name, price, quantity, refCat, ageRestricted));
                }
            }
        } catch (IOException e) {
    ...
    public static void saveInventory(List<Medicine> inventory) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(INVENTORY_FILE))) {
            pw.println("type,id,name,price,quantity,category,extra_field");
            for (Medicine m : inventory) {
                ...
                pw.printf("%s,%s,%s,%.2f,%d,%s,%s\n", 
                        type, m.getMedicineID(), m.getName(), m.getPrice(), 
                        m.getStockQuantity(), m.getCategory().name(), extra);
            }
        } catch (IOException e) {
